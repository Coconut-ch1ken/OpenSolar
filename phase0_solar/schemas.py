"""Phase 0 claim-verification schemas for the Solar overlay.

These dataclasses keep the a4 Phase 0 policy explicit:
claim verdicts describe evidence against the paper claim, while execution
readiness describes whether a run path is operationally prepared.
"""
...
def to_dict(instance: Any) -> dict[str, Any]:
    return _jsonable(asdict(instance))


@dataclass(frozen=True)
class Phase0VerificationRequest:
    request_id: str
    paper_source: str
    repo_source: str
    objective: str
    run_mode: str = "replay_or_approved_execution"
    human_gate_required: bool = True
    source_metadata: dict[str, str] = field(default_factory=dict)
    schema_version: str = SCHEMA_VERSION

    def __post_init__(self) -> None:
        _require_non_empty(self.request_id, "Phase0VerificationRequest.request_id")
        _require_non_empty(self.paper_source, "Phase0VerificationRequest.paper_source")
        _require_non_empty(self.repo_source, "Phase0VerificationRequest.repo_source")
        _require_non_empty(self.objective, "Phase0VerificationRequest.objective")
        _require_non_empty(self.run_mode, "Phase0VerificationRequest.run_mode")
