...
def evaluate(payload: dict[str, Any], path: str | Path | None = None):
    ...
    if execution_mode and execution_mode not in {"fixture", "bounded", "human_approved"}:
        reasons.append("experiment execution_mode must be fixture, bounded, or human_approved")
    if execution_mode == "human_approved" and not plan.get("approval_required"):
        reasons.append("human_approved execution mode requires approval_required=true")
    return finish(payload, reasons, warnings, path=path)
